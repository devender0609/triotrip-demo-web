...
function CheckoutInner() {
  ...
  const total = Number.isFinite(totalRawA)
    ? totalRawA
    : Number.isFinite(totalRawB)
    ? totalRawB
    : undefined;
  ...
  const canSubmit =
    (!!flightId || !!airline) && !!email && !!first && !!last && !!dob;
  ...
  return (
    <main style={{ maxWidth: 960, margin: "16px auto", padding: 12 }}>
      ...
      <section style={card}>
        ...
        <p style={subStyle}>
          Enter passenger and contact details. This is a sandbox flow.
        </p>
        ...
        <div style={summary}>
          ...
          <div><b>Total:</b> {total !== undefined ? fmt.format(Math.round(total)) : "—"} {currency}</div>
        </div>
        ...
        <div>
          <button disabled={!canSubmit} onClick={handlePay} style={btnPrimary}>
            Pay (sandbox) &amp; Book
          </button>
        </div>
      </section>
...
const grid: React.CSSProperties = { display: "grid", gap: 8, gridTemplateColumns: "1fr 1fr" };
