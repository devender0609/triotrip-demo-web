...
export default function ResultCard({
  ...
  const groupedHotels = useMemo(() => {
    ...
    // keep only top 3 per tier — using forEach to avoid iterator requirements
    map.forEach((arr, k) => {
      map.set(k, arr.slice(0, 3));
    });
    // sort tiers: 5★ to 3★ — use Array.from instead of spread on MapIterator
      ...
      {Array.isArray(outSegs) && outSegs.length > 0 && (
        <div style={{ fontSize: 13, color: "#334155" }}>
          ...
          {outSegs.map((s: any, i: number) => {
            ...
            const layover = i < outSegs.length - 1 ? ` — layover before next: ~${Math.max(45, 30 + i * 20)}m` : "";
            return (
              <span key={i}>
                {segText}
                {layover}
      ...
      {Array.isArray(inSegs) && inSegs.length > 0 && (
        <div style={{ fontSize: 13, color: "#334155" }}>
          ...
          {inSegs.map((s: any, i: number) => {
            const segText = `${s.from} → ${s.to}`;
            const layover = i < inSegs.length - 1 ? ` — layover before next: ~${Math.max(45, 30 + i * 20)}m` : "";
            return (
              <span key={i}>
                {segText}
                {layover}
